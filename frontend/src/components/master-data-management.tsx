...
export function MasterDataManagement({ user, isAuthenticated, authLoading }: MasterDataManagementProps) {
  ...
  }, [selectedType, isAuthenticated, user, authLoading, fetchData]);

  // 認証状態が読み込み中の場合は何も表示しない
  if (authLoading) {
    return (
  ...
  if (!isSystemAdmin(user)) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">システム管理者のみアクセス可能です。</p>
      </div>
    );
  }
